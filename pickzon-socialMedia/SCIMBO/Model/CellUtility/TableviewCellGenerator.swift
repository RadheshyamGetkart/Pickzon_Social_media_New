...
class TableviewCellGenerator{
    ...
    public var max_witdh_percent: CGFloat = 0.7 // 0.8
    ...
    fileprivate func getMessageLabelText(_ messageFrame: UUMessageFrame, label: UILabel, cell: CustomTableViewCell, searchedText: String) {
...
//            if(message.message_type == "14")
        ...
        if !searchedText.isEmpty{
            ...
            while let range = payload.range(of: searchedText, options: .regularExpression, range: startIndex..<payload.endIndex) {
                attributed.addAttribute(.backgroundColor, value: UIColor.yellow, range: NSRange(range, in: payload))
                startIndex = range.upperBound
            }
        }
        if messageFrame.message.from == .UUMessageFromMe{
            attributed.addAttributes([NSAttributedString.Key.foregroundColor : UIColor.white, NSAttributedString.Key.font : UIFont.systemFont(ofSize: 15.0)], range: NSMakeRange(0, payload.length))
        }else{
    ...
    func setAsLink(_ foundRange : NSRange?,_ attributed : NSMutableAttributedString) -> NSMutableAttributedString
    {
        if let foundRange = foundRange {
            if foundRange.location != NSNotFound {
                ...
                let colorAttribute = [NSAttributedString.Key.foregroundColor: UIColor(red: 46/255, green: 46/255, blue: 255/255, alpha: 1.0)]
    ...
    fileprivate func setStatusIcon(_ messageFrame: UUMessageFrame!) -> UIImage?
    {
        ...
        if(messageFrame.message.from == MessageFrom(rawValue: 1))
        {
            ...
            if(messageFrame.message.message_status == "1")
            {
                ...
                return UIImage(named: "msg_sent")!
            }
            else if(messageFrame.message.message_status == "2")
            {
                return UIImage(named: "msg_recieved")!
               //return UIImage(named: "doubletick")!
            }
            else if(messageFrame.message.message_status == "3")
            {
                let readReceipts = UserDefaults.standard.string(forKey: "read_receipts") ?? "1"//bool(forKey: "read_receipts")
                return readReceipts.boolValue ? UIImage(named: "msg_seen")! : UIImage(named: "msg_recieved")!
                //UIImage(named: "doubletickgreen")!
            }
            else
            {
    ...
    */
    ...
    fileprivate func storyreplayViewSetter(_ messageFrame: UUMessageFrame, cell:StoryChatTableViewCell){
        ...
        if(Themes.sharedInstance.Getuser_id() ==  messageFrame.message.user_from)
        {
            
            cell.replayName.text = "Replied to Story"
            cell.replaycolour.backgroundColor = .white // UIColor(red:23/255, green:109/255, blue:69/255, alpha:1.0)
            cell.replayName.textColor = .white //UIColor(red:23/255, green:109/255, blue:69/255, alpha:1.0)
            
        }else{
            
            
            cell.replayName.text =  "Replied to Story" //messageFrame.message.replyObj.fromName
            cell.replaycolour.backgroundColor = outgoingBubbleColour //  UIColor.orange
            cell.replayName.textColor = outgoingBubbleColour //UIColor.orange
            
        }
        
        cell.replayImgWidth.constant = 0
        cell.replayImg.isHidden = true
        cell.replayMsg.text = messageFrame.message.storyPayload
        cell.messageLabel.text = messageFrame.message.payload

        if(messageFrame.message.url_str.count > 0)
        {
            cell.replayImg.isHidden = false
            cell.replayImgWidth.constant = 60
            cell.replayImg.kf.setImage(with: URL(string: messageFrame.message.url_str), placeholder: PZImages.dummyCover!, options:  nil, progressBlock: nil) { response in }
        }else if( messageFrame.message.url_str.count == 0)
        {
            cell.replayImgWidth.constant = 0
            cell.replayImg.isHidden = true
        }
    ...
    fileprivate func replayViewSetter(_ messageFrame: UUMessageFrame, cell:ReplayTableViewCell){
        ...
        let messageFromID:String = messageFrame.message.replyObj.fromId//  Themes.sharedInstance.GetsingleDetail(entityname: Constant.sharedinstance.Reply_detail, attrib_name: "doc_id", fetchString: messageFrame.message.doc_id, returnStr: "from_id")
        let message_type:String = messageFrame.message.reply_type // Themes.sharedInstance.GetsingleDetail(entityname: Constant.sharedinstance.Reply_detail, attrib_name: "doc_id", fetchString: messageFrame.message.doc_id, returnStr: "message_type")
        let message:UUMessage! = messageFrame.message
        var payload:String = String()
        ...
        if(Themes.sharedInstance.Getuser_id() == messageFromID)
        {
            ...
            */
            cell.replayName.text = messageFrame.message.replyObj.fromName
        ...
        if(message_type == "1")
        {
          ...
          /*  let ThembnailData:String! = Themes.sharedInstance.GetsingleDetail(entityname: Constant.sharedinstance.Reply_detail, attrib_name: "doc_id", fetchString: messageFrame.message.doc_id, returnStr: "compressed_data")
            ...
            */
            cell.replayImg.isHidden = false
            cell.replayImg.kf.setImage(with: URL(string: messageFrame.message.replyObj.thumbUrl), placeholder: PZImages.dummyCover!, options:  nil, progressBlock: nil) { response in }
                ...
                if let url = URL(string: messageFrame.message.replyObj.thumbUrl){
                    ...
                    DispatchQueue.global(qos: .userInitiated).async {
                        ...
                        DispatchQueue.main.async {
                            cell.replayImg.image = pdfImage
                        }
                    }
                }
            }
            if messageFrame.message.replyObj.payload.count > 0{
                cell.replayMsg.text = messageFrame.message.replyObj.payload
            }
        }else if(message_type == "4")
        {
            cell.replayImg.isHidden = false
            cell.replayImgWidth.constant = 55
            cell.replayImg.kf.setImage(with: URL(string: messageFrame.message.replyObj.locationObj.image), placeholder: PZImages.dummyCover!, options:  nil, progressBlock: nil) { response in }
            cell.replayMsg.text = messageFrame.message.replyObj.locationObj.title
            cell.messageLabel.text = messageFrame.message.payload
        }
       /* else if(message_type == "2")
        {
        ...
        */
        ...
        }else if(message_type == "7")
        {
            cell.replayImgWidth.constant = 0
            cell.replayImg.isHidden = true
            cell.replayMsg.text = message.title_str //  payload
    ...
    fileprivate func contactButtonSetter(_ messageFrame:UUMessageFrame, cell:ContactTableViewCell){
        ...
        if(IsMyContact) {
            ...
            if IsInMyContact, IsUsingApp {
                ...
                cell.singleButtonView.isHidden = true
                cell.saveContactBtn.setTitle(contactTitle.contact.rawValue, for: .normal)
                cell.inviteContatBtn.setTitle(contactTitle.invite.rawValue, for: .normal)
            }
        }
        //By me
...
//        let serverpath:String = UploadHandler.Sharedinstance.ReturnuploadDetails(pathid: messageFrame.message.thumbnail!, upload_detail: "serverpath") as! String
        ...
        let serverpath:String = UploadHandler.Sharedinstance.ReturnuploadDetails(pathid: messageFrame.message.url_str!, upload_detail: "serverpath") as! String

        let param:NSDictionary = ["download_status":"0"]
//        DatabaseHandler.sharedInstance.UpdateData(Entityname: Constant.sharedinstance.Upload_Details, FetchString: messageFrame.message.thumbnail!, attribute: "upload_data_id", UpdationElements: param)
        
        DatabaseHandler.sharedInstance.UpdateData(Entityname: Constant.sharedinstance.Upload_Details, FetchString: messageFrame.message.url_str!, attribute: "upload_data_id", UpdationElements: param)
        ...
        DispatchQueue.global(qos: .background).async {
            if let url = URL(string:serverpath) {
                let upload_PathData = NSData(contentsOf:url)
                DispatchQueue.main.async {
        ...
        DispatchQueue.global(qos: .background).async {
            if let url = URL(string:serverpath) {
                ...
                let time = self.ReturnruntimeDuration(sourceMovieURL:url)
                DispatchQueue.main.async {
    ...
    fileprivate func setAudioCell(_ messageFrame: UUMessageFrame, cell:AudioTableViewCell){
        ...
        let message:UUMessage! = messageFrame.message
        let download_status:String = UploadHandler.Sharedinstance.ReturnuploadDetails(pathid: messageFrame.message.thumbnail!, upload_detail: "download_status") as! String
//        var upload_Path:String = messageFrame.message.url_str // UploadHandler.Sharedinstance.ReturnuploadDetails(pathid: messageFrame.message.thumbnail!, upload_detail: "upload_Path") as! String
        ...
        if(messageFrame.message.from == MessageFrom(rawValue: 1)!)
        {
           /* let upload_status = Themes.sharedInstance.CheckNullvalue(Passed_value: UploadHandler.Sharedinstance.ReturnuploadDetails(pathid: messageFrame.message.url_str, upload_detail: "upload_status"))
            if(upload_status == "0")
            {
                cell.isDownloadInProgress(.indeterminate)
            }
            ...
            if(upload_Path != "")
            {
               ...
               // audioDownloadHandler(messageFrame, cell)
                    ...
                    if let url = URL(string:upload_Path) {
                        if let upload_PathData = NSData(contentsOf:url) as? Data{
                        cell.audioSlider.setValue(0.0, animated: true)
                        cell.audioDuration.text = self.ReturnruntimeDuration(sourceMovieURL:URL(fileURLWithPath:upload_Path))
                        cell.audioDuration.textColor = UIColor.lightGray
                        cell.songData = upload_PathData
                    }
                }
            }
        }
        ...
        cell.audioDuration.text = messageFrame.message.duration
    }
    ...
    fileprivate func SetDocumentDetail(_ messageFrame: UUMessageFrame!, cell:DocTableViewCell)
    {
        ...
        if let localValue = messageFrame.message.docName{
            ...
            fileName = messageFrame.message.url_str.components(separatedBy: "/").last ?? ""
        ...
        if(messageFrame.message.from == MessageFrom(rawValue: 1)!)
        {
            if(messageFrame.message.docType == "1")
            {
                ...
                */
                cell.fileImg.sd_setImage(with: URL(string:messageFrame.message.thumbnail)!)
    ...
    public func returnCell(for tableView:UITableView, messageFrame:UUMessageFrame, indexPath : IndexPath, searchedText: String) -> CustomTableViewCell{
        ...
        case MessageType(rawValue: 0)!:
            ...
            self.getMessageLabelText(messageFrame, label: CustomTableViewCell.messageLabel, cell: CustomTableViewCell, searchedText: searchedText)
            ...
            if calculatedValue.lineCount > 1 || CustomTableViewCell.messageLabel.font == emojiFont{
               // CustomTableViewCell.timeLabelLeadingConstraint.constant = -(CustomTableViewCell.timeLabel.frame.width+10)
                CustomTableViewCell.messageLabelBottom.constant = 25
            }
            else{
               // CustomTableViewCell.timeLabelLeadingConstraint.constant = 5
                CustomTableViewCell.messageLabelBottom.constant = 10
            }
            CustomTableViewCell.setChatviewColor()
        ...
        case MessageType(rawValue: 1)!:
          ...
          // self.imageSetter(messageFrame, in: CustomTableViewCell.chatImg, gifImageView: CustomTableViewCell.gifImg, cell: CustomTableViewCell)
            ...
            CustomTableViewCell.chatImg.kf.setImage(with: URL(string: messageFrame.message.thumbnail), placeholder: PZImages.dummyCover!, options:  nil, progressBlock: nil) { response in
                CustomTableViewCell.chatImg.isHidden = false
                CustomTableViewCell.gifImg.isHidden = true
                
                if messageFrame.isChatRequested == 0{
                    CustomTableViewCell.chatImg.addBlurEffect()
                }
            }
            ...
            if(messageFrame.message.payload.length == 0)
            {
                ...
                CustomTableViewCell.timeLabel.textColor = .darkGray
                CustomTableViewCell.messageLabelBottom.constant =  5 //8
                CustomTableViewCell.messageTop.constant = 0
            }else
            {
                CustomTableViewCell.imageHeightConstaraint.constant = 160
                CustomTableViewCell.timeLabel.textColor = .darkGray //UIColor.black.withAlphaComponent(0.24)
                CustomTableViewCell.messageLabelBottom.constant =  20
                CustomTableViewCell.messageTop.constant = 5
            }
            CustomTableViewCell.setChatviewColor()
            CustomTableViewCell.showVideoSize = false
            
            CustomTableViewCell.downloadIndicator?.isHidden = true
        ...
        case MessageType(rawValue: 2)!:
           ...
           // self.thumbnailSetter(messageFrame, in: CustomTableViewCell.chatImg, cell: CustomTableViewCell)
            ...
            CustomTableViewCell.chatImg.kf.setImage(with: URL(string: messageFrame.message.thumbnail), placeholder: PZImages.dummyCover!, options:  nil, progressBlock: nil) { response in
              ...
              //  CustomTableViewCell.setTrilingConstraint(50, animated: false)
                CustomTableViewCell.hideLoaderView(true)
                CustomTableViewCell.customButton.setImage(#imageLiteral(resourceName: "playIcon.png"), for: .normal)
            }
        ...
        case MessageType(rawValue: 7)!:
            ...
            if calculatedValue.lineCount > 1 || CustomTableViewCell.messageLabel.font == emojiFont{
              ...
              //  CustomTableViewCell.timeLabelLeadingConstraint.constant = 5
                CustomTableViewCell.messageLabelBottom.constant = 5
            }
        ...
        case MessageType(rawValue: 14)!:
            ...
            {
                CustomTableViewCell.imageHeightConstaraint.constant = 160
                CustomTableViewCell.timeLabel.textColor = UIColor.darkGray
        ...
        case MessageType(rawValue: 22)!:
            ...
            var cellId = messageFrame.message.from == MessageFrom(rawValue: 1) ? "outgoingStoryChatTblCell" : "incomingStoryChatTblCell"
            cellId = cellId.appending("")
            let CustomTableViewCell = tableView.dequeueReusableCell(withIdentifier: cellId, for: indexPath) as! StoryChatTableViewCell
            CustomTableViewCell.messageFrame = messageFrame
            CustomTableViewCell.readCount = messageFrame.message.readmore_count ?? ""
            CustomTableViewCell.timeLabel.text = getTimeLabelText(messageFrame)
            self.getMessageLabelText(messageFrame, label: CustomTableViewCell.messageLabel, cell: CustomTableViewCell, searchedText: searchedText)
           ...
           // CustomTableViewCell.timeLabelLeadingConstraint.constant = calculatedValue.lineCount > 1 ? -(CustomTableViewCell.timeLabel.frame.width+10) : 3
            messageFrame.message.messageheight = calculatedValue.height
            CustomTableViewCell.statusImg?.image = self.setStatusIcon(messageFrame)
            storyreplayViewSetter(messageFrame, cell: CustomTableViewCell)
            CustomTableViewCell.setChatviewColor()
            CustomTableViewCell.customButton.setImage(nil, for: .normal)
            CustomTableViewCell.hideLoaderView(true)
            textCell = CustomTableViewCell
