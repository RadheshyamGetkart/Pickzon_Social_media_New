...
//        self.pushView(singleInfoVC, animated: true)
    ...
    func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        ...
        return section == 0 ? "SEEN" : "DELIVERED"
    }
            ...
            if (cellItem?.messageFrame.message.type == MessageType(rawValue: 3)!) {
                ...
                if (cellItem?.btnContent.backImageView != nil)
                {
                    let configuration = ImageViewerConfiguration { config in
                        config.image = cellItem?.btnContent.backImageView.image
                    }
                    let name = cellItem?.UsernameLbl.text ?? ""
                    let timestamp = cellItem?.messageFrame.message.timestamp ?? ""
