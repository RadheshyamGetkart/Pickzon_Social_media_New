...
enum CoinHistoryType{
    ...
    case cheerCoins
    case giftCoins
    case purchaseCoinsHistory
    case referralCoinsHistory
    case agencyTransferHistory
...
class CoinHistoryVC: UIViewController {
    ...
    var coinsTYpe:CoinHistoryType = .cheerCoins
    ...
    override func viewDidLoad() {
        ...
        if coinsTYpe == .purchaseCoinsHistory{
            getCoinTransactionHistoryApi()
            lblNavTitle.text = "Transaction History"
    ...
    @IBAction func optionButtonAction(_ sender:UIButton){
        ...
        let alert = UIAlertController(title: "", message: "", preferredStyle: .actionSheet)
          ...
          alert.addAction(UIAlertAction(title: "Transaction History", style: .default , handler:{ (UIAlertAction)in
              self.pageNumber = 1
              self.coinsTYpe = .purchaseCoinsHistory
              self.self.getCoinTransactionHistoryApi()
              self.lblNavTitle.text = "Transaction History"
              self.btnOption.isHidden = false
          }))
    ...
    //MARK: Appi Methods
    func getCoinTransactionHistoryApi(){
        ...
        let url = Constant.sharedinstance.coin_get_purchase_coins_transaction_history + "?pageNumber=\(pageNumber)"
        if pageNumber == 1 {
    ...
    func getReferralCoinHistoryApi(){
        ...
        let url = Constant.sharedinstance.get_coin_referral_history + "?pageNumber=\(pageNumber)"
        if pageNumber == 1 {
            Themes.sharedInstance.activityView(View: self.view)
        }
        self.isDataLoading = true
            ...
            if(error != nil)
            {
                ...
                if status == 1 {
                    ...
                    if let payload = result["payload"] as? Array<Any>{
                        ...
                        for dict in payload{
                            self.listArray.append(ReferralCoinModel(respDict: dict as! Dictionary<String, Any>))
                        }
    ...
    func getCoinHistoryApi(){
        ...
        var url = Constant.sharedinstance.get_coins_transaction_history  + "?pageNumber=\(pageNumber)&type=1"
        
        if coinsTYpe == .cheerCoins{
             url = Constant.sharedinstance.get_coins_transaction_history  + "?pageNumber=\(pageNumber)&type=2"

        }else if coinsTYpe == .giftCoins{
             url = Constant.sharedinstance.get_coins_transaction_history  + "?pageNumber=\(pageNumber)&type=1"
        }
...
//
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        }else if coinsTYpe == .purchaseCoinsHistory {
            ...
            if let obj =  listArray[indexPath.row] as? CheerCoinModel{
                ...
                cell.lblCoinValue.isHidden = true
                 ...
                 if obj.coinsType  == 0 {
                    ...
                    cell.lblCoinValue.text = obj.coinsDollar
                    cell.lblCoinValue.textColor = .systemGreen
                    cell.lblCoinValue.isHidden = false
...
extension CoinHistoryVC:ExpandableLabelDelegate{
