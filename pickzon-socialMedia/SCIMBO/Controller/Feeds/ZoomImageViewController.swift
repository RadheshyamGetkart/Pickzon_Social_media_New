...
class ZoomImageViewController: UIViewController {
    ...
    @IBAction func downloadBtnAction(_ sender: UIButton) {
        self.downloadAllMedia(urlArray: [imageArrayUrl[currentTag]])
    }
    ...
    func zoomAction(tappedIndex: Int){
        ...
        for i in 0..<imageArrayUrl.count {
            ...
            imageZoom.kf.setImage(with: URL(string: imageArrayUrl[i]), options: nil, progressBlock: nil) { response in
...
extension ZoomImageViewController : UIScrollViewDelegate {
    ...
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        if scrollView.tag == 888888{
            ...
            let fullurl = imageArrayUrl[self.currentTag]
            
            let imghVw = UIImageView()
            imghVw.kf.setImage(with: URL(string: fullurl),placeholder: PZImages.dummyCover,options: [.waitForCache]) { result , error in
                
                self.view.backgroundColor = imghVw.image?.getAverageColour ?? .lightGray
