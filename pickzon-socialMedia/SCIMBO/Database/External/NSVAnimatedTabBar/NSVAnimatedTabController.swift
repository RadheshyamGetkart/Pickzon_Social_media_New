...
public class NSVAnimatedTabController: UIViewController {
     ...
     let animatedTab = AnimatedTab(frame: .zero)
    ...
    public lazy var centerItem = AnimatedTabItem(frame: .zero)
    public var options: NSVAnimatedTabOptions?
    private var subOptionItems: [CenterItemSubOptionItem] = []
    
    var originalConstant = 0
    ...
    public var tabBottomConstraint: NSLayoutConstraint?
    public var centerItemBottomConstraint: NSLayoutConstraint?
    private let _bottomView = UIView(frame: .zero)

    
    // var obj:DefaultCenterItemOptions?
    ...
    func updateOriginalSize(){
        
        self.centerItemBottomConstraint?.constant = CGFloat(originalConstant)
        self.centerItem.sizeThatFits(.init(width: 60, height: 60))
    }
    ...
    func setBadgeString(index:Int){
       // animatedTab.badgebtn.badgeString
    }
    ...
    private func addCenterItem(options: NSVCenterItemOptions) {
        ...
        originalConstant = Int(centerItemBottomConstraint?.constant ?? 0)
        ...
        centerItem.onLongPressed = { sender in
            self.isOpen.toggle()
            self.delegate?.handleLongPressed(sender: sender)
        }
    }
