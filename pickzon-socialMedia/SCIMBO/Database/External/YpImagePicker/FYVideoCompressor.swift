...
public class FYVideoCompressor {
   ...
   // static public var minimumVideoBitrate = 1000 * 200
    static public var minimumVideoBitrate = 1000 * 300
    ...
    public func compressVideo(_ url: URL, quality: VideoQuality = .mediumQuality, outputPath: URL? = nil, completion: @escaping (Result<URL, Error>) -> Void) {
        ...
        // setup
        guard var videoTrack = asset.tracks(withMediaType: .video).first else {
    ...
    func getVideoBitrateWithQuality(_ quality: VideoQuality, originalBitrate: Float) -> Float {
        ...
        if originalBitrate < targetBitrate {
            ...
            case .highQuality:
                ...
                targetBitrate = originalBitrate * 0.7
                targetBitrate = max(targetBitrate, Float(Self.minimumVideoBitrate))
            case .custom(_, _, _):
                break
            }
       ...
       /* let originalWidth = originalSize.width
        ...
        if originalWidth > 720 || originalHeight  > 720  {
            ...
            if originalWidth  > originalHeight {
                width = 720
                height = originalHeight * 720.0 / originalWidth
            }else if originalHeight > originalWidth {
                width = originalWidth * 720.0 / originalHeight
                height = 720.0
            }else {
                width = 720.0
                height = 720.0
        ...
        */
        ...
        var newHeight = 0.0
          ...
          if (originalWidth >= 1920 || originalHeight >= 1920) {
              newWidth = (((originalWidth * 0.5) / 16) * 16)
              newHeight = (((originalHeight * 0.5) / 16) * 16)
           }else if (originalWidth >= 1280 || originalHeight >= 1280){
               newWidth = (((originalWidth * 0.75) / 16) * 16)
               newHeight = (((originalHeight * 0.75) / 16) * 16)
           }/*else if (originalWidth >= 960 || originalHeight >= 960) {
              
               if(originalWidth > originalHeight){
                   newWidth = (Double(originalHeight) * 0.95 / 16) * 16
                   newHeight = (Double(originalWidth) * 0.95 / 16) * 16
               } else {
                   newWidth = (Double(originalWidth) * 0.95 / 16) * 16
                   newHeight = (Double(originalHeight) * 0.95 / 16) * 16
               }
          
           }*/ else{
               //newWidth = (((Double(originalWidth) * 0.9) / 16) * 16)
               //newHeight = (((Double(originalHeight) * 0.9) / 16) * 16)
               newWidth = originalWidth
               newHeight = originalHeight
           }
        
        print("newWidth = (((\(originalWidth) * 0.9) / 16) * 16) :\(newWidth)")
        print("newHeight = (((\(originalHeight) * 0.9) / 16) * 16): \(newHeight)")
        
        return CGSize(width: Int(newWidth), height: Int(newHeight))
