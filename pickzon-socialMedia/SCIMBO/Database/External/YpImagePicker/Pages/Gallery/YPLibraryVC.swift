...
//        v.assetViewContainer.customCropButton
//            .addTarget(self,
...
//
    ...
    @objc
    func customCropButtonTapped() {
        
//        guard let selectedAssetImage = self.v.assetZoomableView.assetImageView.image  else {
//            // If no selected asset, than the squareCropButton is not visible
//            //  squareCropButton.isHidden = true
//            return
//        }
//        let cropViewController = Mantis.cropCustomizableViewController(image: selectedAssetImage)
//        cropViewController.delegate = self
//        cropViewController.modalTransitionStyle = .flipHorizontal
//        self.navigationController?.pushViewController(cropViewController, animated: true)
      //      .presentView(cropViewController, animated: true)
...
//        self.pushView(cropViewController, animated: true)
    ...
    public func selectedMedia(photoCallback: @escaping (_ photo: YPMediaPhoto) -> Void,
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            if self.isMultipleSelectionEnabled && self.selectedItems.count > 1 {
                ...
                if noOfVideos > 1 && selectedAssets.count > 1{
                    DispatchQueue.main.async {
                ...
                for asset in selectedAssets {
                    ...
                    case .video:
                        ...
                        print( asset.asset.pixelWidth, asset.asset.pixelHeight)
                        
                        self.fetchVideoAndApplySettings(for: asset.asset,
                                                        withCropRect: asset.cropRect) { videoURL in
                            if let videoURL = videoURL {
                                
                                if videoURL.sizePerMB() > 5.0 && YPConfig.isToCompressUsingThirdPartyLibrary == true {
                                    
                                    FYVideoCompressor().compressVideo(videoURL, quality: .highQuality) { result in
                                        switch result {
                                        case .success(let compressedVideoURL):
                                            
                                            
                                            print("file size After compression in MB: %f ", compressedVideoURL.sizePerMB())
                                            let videoItem = YPMediaVideo(thumbnail: thumbnailFromVideoPath(compressedVideoURL),
                                                                         videoURL: compressedVideoURL, asset: asset.asset)
                                            videoItem.originalUrl = videoURL
                    ...
                    self.fetchVideoAndApplySettings(for: asset, callback: { videoURL in
                        DispatchQueue.main.async {
                            if let videoURL = videoURL {
                                ...
                                if videoURL.sizePerMB() > 5.0 && YPConfig.isToCompressUsingThirdPartyLibrary == true{
                                    FYVideoCompressor().compressVideo(videoURL, quality: .highQuality) { result in
                                        ...
                                        case .success(let compressedVideoURL):
                                            ...
                                            print("file size After compression in MB: %f ", compressedVideoURL.sizePerMB())
                                            self.delegate?.libraryViewFinishedLoading()
                                            let video = YPMediaVideo(thumbnail: thumbnailFromVideoPath(compressedVideoURL),
                                                                     videoURL: compressedVideoURL, asset: asset)
                                ...
                                ypLog("Problems with fetching videoURL. now original video is used ")
    ...
    func requestConvertAVAsset(asset: PHAsset)-> AVAsset? {
...
extension YPLibraryVC : CropViewControllerDelegate{
    ...
    func cropViewControllerDidCrop(_ cropViewController: Mantis.CropViewController, cropped: UIImage, transformation: Mantis.Transformation, cropInfo: Mantis.CropInfo) {
        self.v.assetZoomableView.assetImageView.image = cropped
//        self.selectedItems[currentlySelectedIndex].cropRect = CGRect(x: 0, y: 0, width: cropInfo.cropSize.width, height: cropInfo.cropSize.height)
//        self.selectedItems[currentlySelectedIndex].scrollViewZoomScale = cropInfo.scale
//        self.selectedItems[currentlySelectedIndex].assetIdentifier = cropInfo.imageViewSize
//
//            guard let asset = PHAsset.fetchAssets(withLocalIdentifiers: [cropped.accessibilityIdentifier],
//                                                  options: PHFetchOptions()).firstObject else {
//                ypLog("Error!")
//                return nil
//            }
