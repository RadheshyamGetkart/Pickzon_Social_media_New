...
open class ImagePickerController: UIViewController {
    ...
    open lazy var galleryView: ImageGalleryView = { [unowned self] in
        let galleryView = ImageGalleryView(config: self.myConfig)
    ...
    open lazy var bottomContainer: BottomContainerView = { [unowned self] in
        let view = BottomContainerView(config: self.myConfig)
        view.backgroundColor = self.myConfig.bottomContainerColor
        view.delegate = self
    ...
    open lazy var topView: TopView = { [unowned self] in
        let view = TopView(config: self.myConfig)
    ...
    lazy var cameraController: CameraView = { [unowned self] in
        let controller = CameraView(config: self.myConfig)
    ...
    public required init(config: MyConfiguration = MyConfiguration()) {
    ...
    open override func viewDidLoad() {
        ...
        view.backgroundColor = myConfig.mainColor
    ...
    open override func viewWillAppear(_ animated: Bool) {
        ...
        if myConfig.managesAudioSession {
    ...
    func presentAskPermissionAlert() {
        let alertController = UIAlertController(title: myConfig.requestPermissionTitle, message: myConfig.requestPermissionMessage, preferredStyle: .alert)
        
        let alertAction = UIAlertAction(title: myConfig.OKButtonTitle, style: .default) { _ in
            if let settingsURL = URL(string: UIApplication.openSettingsURLString) {
                UIApplication.shared.open(settingsURL, options: [:], completionHandler: nil)
            }
        }
        
        let cancelAction = UIAlertAction(title: myConfig.cancelButtonTitle, style: .cancel) { _ in
    ...
    @objc func adjustButtonTitle(_ notification: Notification) {
        ...
        myConfig.doneButtonTitle : myConfig.cancelButtonTitle
    ...
    fileprivate func takePicture() {
        ...
        if myConfig.collapseCollectionViewWhileShot {
...
extension ImagePickerController: CameraViewDelegate {
    ...
    func setFlashButtonHidden(_ hidden: Bool) {
        if myConfig.flashButtonAlwaysHidden {
    ...
    func imageToLibrary(asset : PHAsset) {
        ...
        galleryView.fetchPhotos {
            if self.myConfig.allowMultiplePhotoSelection == false {
