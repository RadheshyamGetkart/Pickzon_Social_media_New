...
open class BottomContainerView: UIView {
  ...
  lazy var pickerButton: ButtonPicker = { [unowned self] in
    let pickerButton = ButtonPicker(config: self.myConfig)
    pickerButton.setTitleColor(UIColor.clear, for: UIControl.State())
    pickerButton.delegate = self
    pickerButton.numberLabel.isHidden = !self.myConfig.showsImageCountLabel
  ...
  lazy var borderPickerButton: UIView = {
    ...
    lazy var UserDescriptionLabel: UILabel = { [unowned self] in
        let label = UILabel()
        label.font = self.myConfig.userDescriptionLabelFont
        label.textColor = self.myConfig.userDisplayLabelColor
        label.text = self.myConfig.userDescriptionLabelText
        label.sizeToFit()
  ...
  open lazy var doneButton: UIButton = { [unowned self] in
    let button = UIButton()
    button.setTitle(self.myConfig.cancelButtonTitle, for: UIControl.State())
    button.titleLabel?.font = self.myConfig.doneButton
  ...
  lazy var topSeparator: UIView = { [unowned self] in
    let view = UIView()
    view.backgroundColor = self.myConfig.backgroundColor
...
//    }
    ...
    backgroundColor = myConfig.backgroundColor
  ...
  @objc func doneButtonDidPress(_ button: UIButton) {
    if button.currentTitle == myConfig.cancelButtonTitle {
