...
open class ImageGalleryView: UIView {
  ...
  lazy var collectionViewLayout: UICollectionViewLayout = { [unowned self] in
    ...
    layout.minimumInteritemSpacing = self.myConfig.cellSpacing
  ...
  lazy var topSeparator: UIView = { [unowned self] in
    ...
    view.backgroundColor = self.myConfig.gallerySeparatorColor
  ...
  open lazy var noImagesLabel: UILabel = { [unowned self] in
    let label = UILabel()
    label.font = self.myConfig.noImagesFont
    label.textColor = self.myConfig.noImagesColor
    label.text = self.myConfig.noImagesTitle
    label.alpha = 0
    label.sizeToFit()
    label.isHidden = true
  ...
  public init(config: MyConfiguration? = nil) {
    if let config1 = config {
      self.myConfig = config1
    }
  ...
  func configure() {
    backgroundColor = myConfig.mainColor
    ...
    topSeparator.addSubview(myConfig.indicatorView)
  ...
  func updateFrames() {
    ...
    topSeparator.autoresizingMask = [.flexibleLeftMargin, .flexibleRightMargin, .flexibleWidth]
      myConfig.indicatorView.frame = CGRect(x: (totalWidth - myConfig.indicatorWidth) / 2, y: (topSeparator.frame.height - myConfig.indicatorHeight) / 2,
      width: myConfig.indicatorWidth, height: myConfig.indicatorHeight)
  ...
  func fetchPhotos(_ completion: (() -> Void)? = nil) {
    AssetManager.fetch(withConfiguration: myConfig) { assets in
...
extension ImageGalleryView: UICollectionViewDelegate {
  ...
  public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
    ...
    if myConfig.allowMultiplePhotoSelection == false {
