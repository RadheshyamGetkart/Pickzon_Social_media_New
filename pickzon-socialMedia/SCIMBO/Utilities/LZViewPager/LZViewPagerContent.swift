...
class LZViewPagerContent: UIView, UIPageViewControllerDelegate, UIPageViewControllerDataSource {
    ...
    func pageViewController(_ pageViewController: UIPageViewController, willTransitionTo pendingViewControllers: [UIViewController]) {
        self.delegate?.willTransition?(to: pendingViewControllers[0].indexx)
    }
    
    func pageViewController(_ pageViewController: UIPageViewController, didFinishAnimating finished: Bool, previousViewControllers: [UIViewController], transitionCompleted completed: Bool) {
        if completed {
            self.currentIndex = (self.pageViewController!.viewControllers![0]).indexx
    ...
    func scroll(to index: Int, animated: Bool = true) {
        if let controller = self.dataSource?.controller(at: index) {
            controller.indexx = index
            if let currentIndex = (self.pageViewController?.viewControllers?[0])?.indexx {
