...
open class AntMediaClient: NSObject, AntMediaClientProtocol {
    ...
    public var localView: RTCVideoRenderer?
    public var remoteView: RTCVideoRenderer?
    ...
    open func isVideoEnabled()->Bool {
        if let videoEnabled = self.webRTCClientMap[self.publisherStreamId ?? (self.p2pStreamId ?? "")]?.isVideoEnabled() {
            return videoEnabled
        }
        return false
    }
    public func sendAudioTrackStatusNotification(enabled:Bool)
    {
